...
3. How to answer to a REST using a HTTP client
...
Vert.x web provides a set of `Handler` for common task such as serving static files:
...
With the SockJS - Eventbus bridge, events from the event bus can be sent to browsers and JAvaScript code running in
...
In (1), we create the `SockJSHandler`. It needs to be configured as by default, for security reason, no messages will
 be transmitted. The set of permitted addresses configure bridge (2). Outbound addresses are for messages from the event
 ...
 client (in the browser) to negotiate the connection and receive / send the messages.

This is not the only bridge that exists for the event bus. There is also a TCP eventbus bridge for native system. The
 SockJS bridge can also be used from Node.JS.
...
It retrieves the audit service from the discovery infrastructure and once it gets the HTTP client. It uses it to call
 the audit REST API. The response is simply send back to the HTTP response. Finally, don't forget to close the HTTP
 client.
