...
instances_ to 2. That means that vert.x is not instantiating this verticle once, with twice (2 different objects).
...
To initialize the trader, write:
...
First create the 2 `Future` objects that will receive the two services once retrieves (1). In (2) we retrieve the
...
. Each translation tries to be as close as possible to the languages idioms, that's why you have `Maps` in Groovy
